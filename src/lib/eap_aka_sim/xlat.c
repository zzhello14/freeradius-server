						   ...
						   REQUEST *request, char const *fmt)
{
	...
	if (type_hint != AKA_SIM_ID_TYPE_PERMANENT) {
		REDEBUG2("AKA/SIM Id \"%pV\" is not a permanent identity (IMSI)", fr_box_strvalue_len(id, id_len));
		goto error;
	}
	 ...
	 */
	if (fr_aka_sim_id_3gpp_pseudonym_encrypt(encrypted, id + 1, id_len - 1, tag, (uint8_t)key_index, key) < 0) {
		RPEDEBUG2("Failed encrypting AKA/SIM Id \"%pV\"", fr_box_strvalue_len(id, id_len));
		return -1;
	}
